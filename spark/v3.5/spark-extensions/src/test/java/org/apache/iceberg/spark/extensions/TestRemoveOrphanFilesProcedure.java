...
public class TestRemoveOrphanFilesProcedure extends SparkExtensionsTestBase {
  ...
  public void testRemoveOrphanFilesInDataFolder() throws IOException {
    ...
    assertThat(output2).as("Should be 1 orphan file in the data folder").hasSize(1);
    ...
    List<Object[]> output1 =
        sql(
            "CALL %s.system.remove_orphan_files("
                ...
                + "dry_run => true)",
            catalogName, tableIdent, currentTimestamp);
    assertThat(output1).as("Should be one orphan file").hasSize(1);
    ...
    List<Object[]> output2 =
        sql(
            "CALL %s.system.remove_orphan_files("
                ...
                + "older_than => TIMESTAMP '%s')",
            catalogName, tableIdent, currentTimestamp);
    assertThat(output2).as("Should be one orphan file").hasSize(1);
    ...
    List<Object[]> output3 =
        sql(
            "CALL %s.system.remove_orphan_files("
                ...
                + "older_than => TIMESTAMP '%s')",
            catalogName, tableIdent, currentTimestamp);
    assertThat(output3).as("Should no more orphan files").hasSize(0);
  ...
  public void testConcurrentRemoveOrphanFiles() throws IOException {
    ...
    assertThat(output).as("Should be 4 orphan files in the data folder").hasSize(4);
  ...
  public void testRemoveOrphanFilesProcedureWithPrefixMode()
    ...
    if (catalogName.equals("testhadoop")) {
      ...
      sql(
          "CREATE TABLE %s (id bigint NOT NULL, data string) USING iceberg LOCATION '%s'",
          tableName, java.nio.file.Files.createTempDirectory(temp, "junit").toUri().toString());
    }
    Table table = Spark3Util.loadIcebergTable(spark, tableName);
    ...
    assertThat(orphanFiles).hasSize(0);
